...
;; Expectations:
(check-expect (draw-car 0) (place-image CAR -40 CAR-Y BACKGROUND))
(check-expect (draw-car 50) (place-image CAR 10 CAR-Y BACKGROUND))
(check-expect (draw-car 200) (place-image CAR 160 CAR-Y BACKGROUND))
(define (draw-car car-position)
  (place-image CAR (- car-position (/ CAR-LENGTH 2)) CAR-Y BACKGROUND))
...
;; move-with-mouse: CarPosition Number Number String -> CarPosition
;; Change the position of the car to the position of x-mouse if the
;; given mouse-event is "button-down"
(check-expect (move-with-mouse 21 10 20 "enter") 21)
(check-expect (move-with-mouse 42 10 20 "button-down") 10)
(check-expect (move-with-mouse 42 10 20 "move") 42)
(define (move-with-mouse car-position x-mouse y-mouse mouse-event)
  (cond
